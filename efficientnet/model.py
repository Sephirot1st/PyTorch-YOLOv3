...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None):
        self.buffered_feature_maps8 = None
        self.buffered_feature_maps11 = None
    ...
    def extract_features(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            # print("MB ", idx, " input ", x.size())

            drop_connect_rate = self._global_params.drop_connect_rate
            if drop_connect_rate:
                drop_connect_rate *= float(idx) / len(self._blocks)
            x = block(x)  # , drop_connect_rate) # see https://github.com/tensorflow/tpu/issues/381
            # print("MB ", idx, "output", x.size())
            if idx == 8:
                self.buffered_feature_maps8 = x
            elif idx == 11:
                self.buffered_feature_maps11 = x
